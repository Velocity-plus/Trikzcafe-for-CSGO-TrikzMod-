...
from .tcore.instances import shared_path
...
def start_ignore_anti_stuck(player, time):
    player.ignore_anti_stuck = True
    if player.ignore_anti_stuck_timer:
        player.ignore_anti_stuck_timer.cancel()
    player.ignore_anti_stuck_timer = player.delay(time, end_ignore_anti_stuck, args=(player,))


def end_ignore_anti_stuck(player):
    player.ignore_anti_stuck = False
    player.ignore_anti_stuck_timer = None
...
def start_touch_func(args):
    SayText2('Is antistuck enabled').send()
    ...
    if player.velocity.length == 0.0 or target.velocity.length == 0.0 and not player.ignore_anti_stuck:
