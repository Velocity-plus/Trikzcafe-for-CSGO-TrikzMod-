...
manager = TypeManager()
CBaseEntity = manager.create_type_from_file('CBaseEntity', Path(__file__).parent / 'hooks/CBaseEntity.ini')
SayText2(str(Path(__file__).parent / 'hooks/CBaseEntity.ini')).send()
...
def _pre_pass_server_entity_filter(args):
    ...
    if base_entity1.classname == "flashbang_projectile":
        ...
        if index0 in HITBOX:
            other = PLAYER[HITBOX[index0].parent.index]
            player = PLAYER[owner_index]
        ...
        if owner_index in level.finished:
            #SayText2('Coll %s' % func_properties[index0]['collision']).send()
    ...
    #SayText2(str(base_entity1.classname) + " | " + str(base_entity0.classname)).send()
    #SayText2(str(func_properties[index0])).send()
...
def pre_set_transmit_player(args):
    ...
    if "projectile" in entity_classname:
        ...
        other = PLAYER[entity.owner.index]
        if player.realm != other.realm:
            return False
