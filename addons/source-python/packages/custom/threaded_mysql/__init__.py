...
class ThreadedMySQL:
    ...
    def execute(self, query, args=None, callback=None, data_pack=None, prioritize=False, get_info=False):
        ...
        if get_info:
            get_info = {'query': query, 'time': current_time, 'prioritized': prioritize}

        signature_encode = sha256(str(current_time).encode())
        signature = signature_encode.hexdigest()
        if not prioritize:
            self._r_queue.put([query, args, callback, data_pack, get_info, query_type, signature])
        else:
            self._p_queue.put([query, args, callback, data_pack, get_info, query_type, signature])
