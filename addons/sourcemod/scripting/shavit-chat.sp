...
public void OnPluginStart()
{
	...
	gCV_CustomChat = CreateConVar("shavit_chat_customchat", "0", "Allow custom chat names or message colors?\n0 - Disabled\n1 - Enabled (requires chat flag/'shavit_chat' override)\n2 - Allow use by everyone", 0, true, 0.0, true, 2.0);
...
public Action Hook_SayText2(UserMsg msg_id, any msg, const int[] players, int playersNum, bool reliable, bool init)
{
	...
	if((HasCustomChat(client) || GetAdminFlag(GetUserAdmin(client), Admin_Root)) && gI_ChatSelection[client] == -1)
	{
...
{
	if((HasCustomChat(client) || GetAdminFlag(GetUserAdmin(client), Admin_Root)))
	{
...
{
	if(rank == -2 ||
		(rank == -1 && (HasCustomChat(client) || GetAdminFlag(GetUserAdmin(client), Admin_Root))))
	{
		return true;
	}
...
void FormatColors(char[] buffer, int size, bool colors, bool escape)
{
	if(colors)
	{
		...
		ReplaceString(buffer, size, "{red}", "\x07");
		ReplaceString(buffer, size, "{lightred}", "\x0F");
		ReplaceString(buffer, size, "{darkred}", "\x02");
		ReplaceString(buffer, size, "{bluegrey}", "\x0A");
		ReplaceString(buffer, size, "{blue}", "\x0B");
		ReplaceString(buffer, size, "{darkblue}", "\x0C");
		ReplaceString(buffer, size, "{purple}", "\x03");
		ReplaceString(buffer, size, "{orchid}", "\x0E");
		ReplaceString(buffer, size, "{yellow}", "\x09");
		ReplaceString(buffer, size, "{gold}", "\x10");
		ReplaceString(buffer, size, "{lightgreen}", "\x05");
		ReplaceString(buffer, size, "{green}", "\x04");
		ReplaceString(buffer, size, "{lime}", "\x06");
		ReplaceString(buffer, size, "{grey}", "\x08");
		ReplaceString(buffer, size, "{grey2}", "\x0D");
...
void FormatRandom(char[] buffer, int size)
{
	...
	do
	{
		FormatEx(temp, 8, "%s", RandColors[GetRandomInt(0, sizeof(RandColors)-1)]);
	}
